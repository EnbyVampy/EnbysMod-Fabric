...
//        matrices.push();
//
//        // Position the text slightly above or below the center for visual clarity
//        matrices.translate(0.5, 0.25, 0.5); // Center (X and Z), Y slightly lower than center
//
//        // Rotate the text so it faces the player
//        Camera camera = MinecraftClient.getInstance().gameRenderer.getCamera();
//        matrices.multiply(camera.getRotation());
//        matrices.scale(-0.025f, -0.025f, 0.025f); // Scale to make text small and readable
//
//        // Set up the text renderer and draw the text
...
//    }
    ...
    public void render(GenericDisplayBlockEntity entity, float tickDelta, MatrixStack matrices,
        ...
        if (!stack.isEmpty()) {
            matrices.push();
//pedestal
        // Position the item to the center of the block
            matrices.translate(0.5, 0.25, 0.5); // X, Y, Z center of the block

        // Add optional floating or rotation animations here if desired
            float rotation = (System.currentTimeMillis() / 20) % 360; // Smooth rotation
            matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(rotation));

        // Adjust the item's scale to fit nicely in the display
